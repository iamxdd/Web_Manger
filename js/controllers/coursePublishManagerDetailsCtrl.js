app.controller('coursePublishManagerDetailsCtrl', ['$scope', '$stateParams', '$state', '$q', '$rootScope', '$http', 'ngDialog', 'PagerExtends', 'layerAlert', '$filter', 'PcService', 'serverUrls',
	...
	//	paramData = paramData.Traning;
		...
		$scope.trainingById = function() {
			$scope.listBusyPromise = $http({
				...
				method: "get",
				url: serverUrls.trainingById + "?id=" + Id
			}).success(function(response) {
		...
		//发布
		$scope.publishItem = function(id, x) {
			switch (id) {
				case 1:
					$scope.listBusyPromise = $http({
						headers: $rootScope.pHeader,
					...
					break;
				case 2:
					$state.go("app.coursePublishManager");
					break;
				default:
					break;
			}
