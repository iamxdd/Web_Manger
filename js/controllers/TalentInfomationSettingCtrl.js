app.controller('TalentInfomationSettingCtrl', ['$scope', '$state', '$q', '$rootScope', '$http', 'ngDialog', 'PagerExtends', 'layerAlert', '$filter', 'PcService', 'serverUrls',
	function($scope, $state, $q, $rootScope, $http, ngDialog, PagerExtends, layerAlert, $filter, PcService, serverUrls) {
		...
		$scope.fetchData = function(isDelete) {
			//PcService.fetchList($scope, serverUrls.getcommontaglist);
		...
		$scope.deleteItem = function(x) { //
			$scope.listBusyPromise = $http({
				headers: $rootScope.pHeader,
				method: "delete",
				url: serverUrls.deletecommontagbyid + "?id=" + x.Id
			}).success(function(response) {
				var Code = response.State.Code;
				var Message = response.State.Message;
				if (Code === 0) {
		...
		$scope.blurFun = function(x) {
				...
				$scope.listBusyPromise = $http({
					headers: $rootScope.pHeader,
					method: "put",
					url: serverUrls.updatecommontag,
					data: _param
				}).success(function(response) {
					var Code = response.State.Code;
					var Message = response.State.Message;
