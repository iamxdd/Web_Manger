app.controller('infomationBrowseStatisticsCtrl', ['$scope', '$state', '$q', '$rootScope', '$http', 'ngDialog', 'PagerExtends', 'layerAlert', '$filter', 'PcService', 'serverUrls',
	function($scope, $state, $q, $rootScope, $http, ngDialog, PagerExtends, layerAlert, $filter, PcService, serverUrls) {
		...
		$scope.fetchData = function() {
			...
			PcService.fetchData($scope, serverUrls.statisticslist, $scope.searchOption, $rootScope.pHeader);		
		};
		//	$scope.fetchData();
		//查看详情
		$scope.seeDetail = function(x) {
			$scope.DetailsData = JSON.stringify(x);
			$state.go("app.infomationBrowseStatisticsDetails", {
